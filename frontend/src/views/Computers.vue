...
const exportComputers = async () => {
  try {
    ...
    const templateResponse = await fetch('/TEMPLATE - ADMISSION DEPARTMENT.xlsx')
    if (!templateResponse.ok) {
      throw new Error('Template file not found')
    }
    
    // Convert template to array buffer
    const templateArrayBuffer = await templateResponse.arrayBuffer()
    
    // Read the template workbook
    const wb = XLSX.read(templateArrayBuffer, { type: 'array' })
    
    // Get the first worksheet (assuming it's the main template)
    const templateWs = wb.Sheets[wb.SheetNames[0]]
    
    // Prepare data for Excel export matching the template format
    const exportData = computersToExport.map((computer, index) => ([
      index + 1, // No.
      computer.computer_name || 'N/A',
      computer.asset_tag || 'N/A',
      computer.pc_number || 'N/A',
      computer.serial_number || 'N/A',
      computer.department?.name || 'Academic Use',
      computer.processor?.model || 'No Processor',
      computer.motherboard?.model || 'No Motherboard',
      computer.ram?.capacity || 'No RAM',
      computer.storage?.capacity || 'No Storage',
      computer.video_card?.model || 'No Video Card',
      computer.psu?.wattage ? `${computer.psu.wattage}W` : 'No PSU',
      computer.dvd_rom?.model || 'No DVD ROM',
      computer.status || 'Working',
      computer.assigned_user?.name || 'Unassigned',
      computer.location || 'Not specified',
      computer.description || 'No description available',
    ...
    // Copy column widths from template if available
    if (templateWs['!cols']) {
    ...
    // Copy styling from template if available
    if (templateWs['!ref']) {
      const templateRange = XLSX.utils.decode_range(templateWs['!ref'])
      for (let row = templateRange.s.r; row <= Math.min(templateRange.e.r, 0); row++) {
        for (let col = templateRange.s.c; col <= templateRange.e.c; col++) {
          const templateCellAddress = XLSX.utils.encode_cell({ r: row, c: col })
          const templateCell = templateWs[templateCellAddress]
          if (templateCell && templateCell.s) {
            const newCellAddress = XLSX.utils.encode_cell({ r: row, c: col })
            if (!ws[newCellAddress]) ws[newCellAddress] = {}
            ws[newCellAddress].s = templateCell.s
          }
        }
      }
    }
    
    // Replace the first sheet with our data
    wb.Sheets[wb.SheetNames[0]] = ws
    wb.SheetNames[0] = 'Computer Inventory'
    
    // Add summary sheet
    const summaryData = [
      ['BASIC INFORMATION', '', ''],
      ['Total Computers', totalComputers.value, ''],
      ['Working Computers', workingComputers.value, ''],
      ['Defective Computers', defectiveComputers.value, ''],
      ['For Disposal', disposalComputers.value, ''],
      ['', '', ''],
      ['COMPONENT STATISTICS', '', ''],
      ['Processors', componentStats.value.processors, ''],
      ['RAM Modules', componentStats.value.ram, ''],
      ['Storage Devices', componentStats.value.storage, ''],
      ['Video Cards', componentStats.value.video_cards, ''],
      ['', '', ''],
      ['EXPORT INFORMATION', '', ''],
      ['Export Date', new Date().toLocaleDateString(), ''],
      ['Export Time', new Date().toLocaleTimeString(), ''],
      ['Exported By', 'MIS Inventory System', '']
    ]
    ...
    for (let row = 0; row < summaryData.length; row++) {
      if (summaryData[row][0] && (summaryData[row][0].includes('INFORMATION') || 
          summaryData[row][0].includes('STATISTICS'))) {
